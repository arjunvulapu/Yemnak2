...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([self.arr containsObject:[self.areas objectAtIndex:indexPath.row]]){
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UITableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:@"cell"];
    if([_arr containsObject:[self.areas objectAtIndex:indexPath.row]]){
        [self.arr removeObject:[self.areas objectAtIndex:indexPath.row]];
        
    }else{
        [self.arr addObject:[self.areas objectAtIndex:indexPath.row]];
    }
    [_tableView reloadData];
    //    self.completionBlock(self.areas);
    //    [self.delegate cancelButtonClicked:self];
}


- (IBAction)doneBtnAction:(id)sender {
    self.completionBlock(self.arr);
    [self.delegate cancelButtonClicked:self];
}
@end
